...
from transformers import  AutoProcessor, AutoModel,pipeline,Data2VecAudioForCTC, Wav2Vec2Processor, Wav2Vec2CTCTokenizer, AutoTokenizer
import torch
...
def main(args):
    ...
    arch = 'data2vec-audio-large-960h'  #'data2vec-audio-large-960h'
    dataset_name = 'svarah' #or saa_l1
    owner = 'facebook'
    split_name = 'audio' 

    tokenizer_wav2vec = AutoTokenizer.from_pretrained(f"{owner}/{arch}")  #Wav2Vec2CTCTokenizer

    model_name = f'{arch}-{owner}-{dataset_name}-{split_name}'
    data2vec_large_asr = pipeline(
                "automatic-speech-recognition", model=f"{owner}/{arch}", device=device, tokenizer =tokenizer_wav2vec   #change
        )
