...
from transformers import  AutoProcessor, AutoModel,pipeline,WavLMForCTC
import torch
...
def main(args):
    ...
    arch = 'wavlm-libri-clean-100h-large'
    dataset_name = '' 
    owner = 'patrickvonplaten'
    split_name = 'audio' 
    model_name = f'{arch}-{owner}-{dataset_name}-{split_name}' 
    wavlm_large_asr = pipeline(
                "automatic-speech-recognition", model=f"{owner}/{arch}", device=device 
        )

    processor = AutoProcessor.from_pretrained(f"{owner}/{arch}") #change
    model = WavLMForCTC.from_pretrained(f"{owner}/{arch}")
    ...
    for i in tqdm(range(len(dataset))):
        op = wavlm_large_asr(dataset[i]['raw'])['text']
        # if len(op) == 0:
        #     continue
